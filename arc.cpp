...
atom make_cons(atom car_val, atom cdr_val)
{
	atom a;
	a.type = T_CONS;
	a.p = make_shared<cons>(car_val, cdr_val);
	return a;
}
...
error builtin_less(atom args, atom *result)
{
	...
	case T_STRING:
		while (!no(cdr(args))) {
			if ((car(args).as<string>()) >= (car(cdr(args)).as<string>())) {
				*result = nil;
				return ERROR_OK;
			}
			args = cdr(args);
...
error builtin_read(atom args, atom *result) {
	...
	if (alen == 0) {
		...
		const char *buf = car(args).as<string>().c_str();
		error err = read_expr(buf, &buf, result);
		return err;
	}
	else return ERROR_ARGS;
...
error builtin_system(atom args, atom *result) {
	...
	if (alen == 1) {
		...
		*result = make_number(system(car(args).as<string>().c_str()));
		return ERROR_OK;
	}
	else return ERROR_ARGS;
}
...
error builtin_infile(atom args, atom *result) {
	if (len(args) == 1) {
		atom a = car(args);
		if (a.type != T_STRING) return ERROR_TYPE;
		FILE *fp = fopen(a.as<string>().c_str(), "r");
...
error builtin_outfile(atom args, atom *result) {
	if (len(args) == 1) {
		atom a = car(args);
		if (a.type != T_STRING) return ERROR_TYPE;
		FILE *fp = fopen(a.as<string>().c_str(), "w");
...
error builtin_coerce(atom args, atom *result) {
	...
	case T_NUM:
		...
		break;
	case T_STRING:
		if (is(type, sym_sym)) *result = make_sym(obj.as<string>().c_str());
		else if(is(type, sym_cons)) {
			*result = nil;
			int i;
			for (i = strlen(obj.as<string>().c_str()) - 1; i >= 0; i--) {
				*result = make_cons(make_char(obj.as<string>().c_str()[i]), *result);
			}
		}
		else if(is(type, sym_num)) *result = make_number(atof(obj.as<string>().c_str()));
		else if(is(type, sym_int)) *result = make_number(atoi(obj.as<string>().c_str()));
		else if (is(type, sym_string))
			*result = obj;
		else
			return ERROR_TYPE;
...
error builtin_len(atom args, atom *result) {
	...
	if (a.type == T_CONS) {
		...
		*result = make_number(strlen(a.as<string>().c_str()));
	}
